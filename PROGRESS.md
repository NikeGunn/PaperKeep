...
| 2026-04-11 | 1C.1–1C.5 | **PHASE 1C COMPLETE.** android-ci.yml (lint+test+assemble+artifact+instrumented matrix api 26/30/34). backend-ci.yml (vet+staticcheck+gosec SARIF+govulncheck+race test+coverage). backend-deploy-staging.yml (CGO_ENABLED=0 cross-compile, SSH deploy, migration, smoke test, Telegram notify). security-scan.yml (OWASP SARIF, govulncheck, TruffleHog --only-verified). intelligence-ci.yml verified (YAML valid). All 5 YAML files pass yaml.safe_load. No hardcoded secrets. All credentials use ${{ secrets.* }}. |
